...
def fig(mean,std,mean1,std1):
    ...
    ylabel='Dropout Aggregation Time(s)'
...
#     axins = inset_axes((0.4, 0.1, 0.4, 0.3))
    axins = ax.inset_axes((0.74, 0.07, 0.25, 0.2))
    for i in range(len(new_mean)):
...
# 坐标轴的扩展比例（根据实际数据调整）
    ...
    plt.savefig('dropout_aggregation_time_num.pdf',dpi=600)
    plt.close()



if __name__=='__main__':
    filename='./dropout_aggregation_time_num.txt'
    filename1='/home/b1107/user/ct/code/multi-IBE/secureaggregation/dropout_aggregation_time_num.txt'
