...
def fig(mean,std):
    ...
    ax.set_yticks(np.arange(0,0.005,0.0005))
    # y=np.arange(0,0.005,0.0005)
    ...
    # plt.tight_layout()
    # plt.ticklabel_format(style='scientific',scilimits=(-3,1), axis='y',useMathText=True,la)
 
    plt.subplots_adjust(left=0.18, bottom=0.15)
    plt.savefig('setup.pdf',dpi=600)
    plt.close()
