...
def Figure1(mean,std,filename):
    ...
    marker=['.', '.', '.', '.']
    font_size=16
    font2={'weight':'semibold',
      'size':16}
    font3={
    'weight':'semibold',
      'size':15}
    label=['No Dropout','20% Dropout I','40% Dropout I','60% Dropout I']
    plt.title("MNIST-LetNet",fontdict=font2)
    xlabel='Training Rounds '
    ylabel='Model Accuracy(%)'    
    # legend=''
    for i in range(len(mean)):
        plt.plot(x,mean[i],linewidth = 1.5, color = color[i], marker=marker[i],linestyle = linestyle[i], label = label[i])
        plt.fill_between(x,mean[i]-std[i],mean[i]+std[i],color=color[i],alpha=0.3)
    plt.xticks(np.arange(5,45,5),size=12,weight='semibold')
    plt.yticks(np.arange(0,110,10),size=12,weight='semibold')
...
def show():
    ...
    Figure1(acc_mean,acc_std,"droptype1acc_mnist.pdf")
    Figure2(acc_mean1,acc_std1,"droptype2acc_mnist.pdf")
